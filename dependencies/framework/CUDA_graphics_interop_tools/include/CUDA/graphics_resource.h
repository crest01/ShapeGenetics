...
namespace CU
{
	namespace graphics
	{
		...
		typedef scoped_handle<CUgraphicsResource, nullptr, GraphicsUnregisterResourceDeleter> scoped_resource;
		typedef unique_handle<CUgraphicsResource, nullptr, GraphicsUnregisterResourceDeleter> unique_resource;
		...
		public:
			...
			map_resources(CUgraphicsResource* resources, unsigned int count, CUstream stream = 0)
				: resources(resources),
				  ...
				  stream(stream)
			{
				checkError(cuGraphicsMapResources(count, resources, stream));
			}
			...
			~map_resources()
			{
				checkError(cuGraphicsUnmapResources(count, resources, stream));
			}
		};

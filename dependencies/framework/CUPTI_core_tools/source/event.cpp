...
namespace CUPTI
{
	namespace secret
	{
		...
		T getEventDomainAttribute(CUpti_EventDomainID event_domain, CUpti_EventDomainAttribute attribute)
		{
			...
			checkError(cuptiEventDomainGetAttribute(event_domain, attribute, &size, &value));
			return value;
		}
		...
		T getEventDomainAttribute(CUdevice device, CUpti_EventDomainID event_domain, CUpti_EventDomainAttribute attribute)
		{
			T value;
			size_t size = sizeof(value);
			checkError(cuptiDeviceGetEventDomainAttribute(device, event_domain, attribute, &size, &value));
			return value;
		}
		...
		T getEventAttribute(CUpti_EventID event, CUpti_EventAttribute attribute)
		{
			T value;
			size_t size = sizeof(value);
			checkError(cuptiEventGetAttribute(event, attribute, &size, &value));
			return value;
		}
		...
		T getEventGroupAttribute(CUpti_EventGroup event_group, CUpti_EventGroupAttribute attribute)
		{
			T value;
			size_t size = sizeof(value);
			checkError(cuptiEventGroupGetAttribute(event_group, attribute, &size, &value));
			return value;
		}
	...
	std::vector<CUpti_EventDomainID> enumEventDomains(CUdevice device)
	{
		...
		if (num_domains)
		{
			...
			checkError(cuptiDeviceEnumEventDomains(device, &domain_buffer_size, &domains[0]));
	...
	{
		uint32_t num_events;
		checkError(cuptiEventDomainGetNumEvents(domain, &num_events));
		return num_events;
	}
	...
	std::vector<CUpti_EventID> enumEventsInDomain(CUpti_EventDomainID domain)
	{
		...
		if (num_events)
		{
			...
			checkError(cuptiEventDomainEnumEvents(domain, &event_buffer_size, &events[0]));
	...
	CUpti_EventID getEventID(CUdevice device, const char* name)
	{
		...
		checkError(cuptiEventGetIdFromName(device, name, &event_id));
		return event_id;
	}
	...
	EventGroup createEventGroup(CUcontext context)
	{
		...
		checkError(cuptiEventGroupCreate(context, &event_group, 0U));
		return event_group;
	}
}

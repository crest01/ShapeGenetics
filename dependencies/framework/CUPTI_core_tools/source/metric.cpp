...
namespace CUPTI
{
	namespace secret
	{
		...
		T getMetricAttribute(CUpti_MetricID metric, CUpti_MetricAttribute attribute)
		{
			...
			checkError(cuptiMetricGetAttribute(metric, attribute, &size, &value));
			return value;
		}
	...
	uint32_t getNumMetrics()
	{
		...
		checkError(cuptiGetNumMetrics(&num_metrics));
		return num_metrics;
	}
	...
	{
		uint32_t num_metrics;
		checkError(cuptiDeviceGetNumMetrics(device, &num_metrics));
		return num_metrics;
	}
	...
	std::vector<CUpti_MetricID> enumMetrics()
	{
		...
		if (num_metrics)
		{
			...
			checkError(cuptiEnumMetrics(&metric_buffer_size, &metrics[0]));
	...
	std::vector<CUpti_MetricID> enumMetrics(CUdevice device)
	{
		...
		{
			size_t metric_buffer_size = sizeof(CUpti_MetricID) * metrics.size();
			checkError(cuptiDeviceEnumMetrics(device, &metric_buffer_size, &metrics[0]));
			metrics.resize(metric_buffer_size / sizeof(CUpti_MetricID));
		}
		return metrics;
	}
	...
	CUpti_MetricID getMetricID(CUdevice device, const char* name)
	{
		...
		checkError(cuptiMetricGetIdFromName(device, name, &metric_id));
		return metric_id;
	}
	...
	{
		uint32_t num_events;
		checkError(cuptiMetricGetNumEvents(metric, &num_events));
		return num_events;
	}
	...
	std::vector<CUpti_EventID> enumEventsInMetric(CUpti_MetricID metric)
	{
		...
		if (num_events)
		{
			...
			checkError(cuptiMetricEnumEvents(metric, &event_buffer_size, &events[0]));
	...
	uint32_t getNumProperties(CUpti_MetricID metric)
	{
		...
		checkError(cuptiMetricGetNumProperties(metric, &num_properties));
	...
	std::vector<CUpti_MetricPropertyID> enumProperties(CUpti_MetricID metric)
	{
		...
		if (num_properties)
		{
			...
			checkError(cuptiMetricEnumProperties(metric, &property_buffer_size, &properties[0]));
